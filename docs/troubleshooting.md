...
| --- | --- | --- |
| Preflight recusa com `no executor is registered for it` | o estágio não pôde ser composto da configuração | é informação, não defeito: `compose_executors()` deixa de fora um estágio cujos pontos de variação não estão todos selecionados (o preflight aponta o componente sem backend), cujo backend precisa de módulo ou segredo ausente (idem), ou que não tem executor. Veja a tabela de estado em [PIPELINE.md](PIPELINE.md) |
| Preflight recusa com `components.<capability>.<slot>: no executor could be composed for stage ...` | o backend recusou os próprios parâmetros, falta o runtime de modelo (provider) ou o alvo `resources.providers` é inválido | a mensagem traz a causa da capability (parâmetro e motivo, ou alvo e motivo); corrija a configuração ou forneça o provider. Nada substitui o estágio |
| Preflight recusa em `components.spatial_relations.candidate` com `the candidate reach does not cover the evaluators' tolerances` | cada política é válida sozinha, mas o `proximity_radius_m` dos candidatos é menor que uma tolerância de um avaliador selecionado (`next_to_max_gap_m`, `2 * containment_slack_m` ou `contact_distance_m + contact_tolerance_m`): pares que o avaliador aceitaria seriam excluídos antes de medidos | aumente `proximity_radius_m` ou reduza a tolerância nomeada na mensagem, no componente `geometric_predicate` ou `contact_predicate`. Ver [coerência entre alcance e tolerâncias](../src/contextmap/spatial_relations/docs/candidates.md#coerência-entre-alcance-e-tolerâncias-dos-avaliadores) |
