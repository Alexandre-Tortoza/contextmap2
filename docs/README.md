...
Spatial Relations descreve **como entidades resolvidas se relacionam no espaço, sem esconder a incerteza**: possui `Relation` (predicado canônico entre duas `ResolvedEntityReference`, com estado `SUPPORTED`, `REJECTED` ou `UNRESOLVED` e o rastro da evidência), `RelationEvidence` (o que um canal geométrico, de contato por pontos ou de observação mediu ou afirmou sobre um candidato dirigido, com medições, limiares, geometria e proveniência), a taxonomia versionada de 11 predicados (direção, simetria, inverso e requisito de frame), as convenções de frame declaradas pela execução (nada a montante define o eixo vertical), a geração determinística de candidatos, os avaliadores geométricos e de contato, a evidência de observação (afirmações upstream como canal só corroborante), a política de decisão conservadora com consistência de inverso e simetria, o `SpatialRelationsRunArtifact` e o avaliador por predicado em `evaluation`. Depende só do contrato mínimo `ResolvedEntityReference` de Entity Resolution: o vínculo automático de afirmações a entidades resolvidas e a linhagem derivada do manifest de resolução ficam para a integração. Toda a verificação usa fixtures sintéticos, sem run de relações sobre dados reais nem reference set real de relações.
