...
Spatial Relations descreve **como entidades resolvidas se relacionam no espaço, sem esconder a incerteza**: possui `Relation` (predicado canônico entre duas `ResolvedEntityReference`, com estado `SUPPORTED`, `REJECTED` ou `UNRESOLVED` e o rastro da evidência), `RelationEvidence` (o que um canal geométrico, de contato por pontos ou de observação mediu ou afirmou sobre um candidato dirigido, com medições, limiares, geometria e proveniência), a taxonomia versionada de 11 predicados (direção, simetria, inverso e requisito de frame), as convenções de frame declaradas pela execução (nada a montante define o eixo vertical), a geração determinística de candidatos, os avaliadores geométricos e de contato, a evidência de observação (afirmações upstream como canal só corroborante), a política de decisão conservadora com consistência de inverso e simetria, o `SpatialRelationsRunArtifact` e o avaliador por predicado em `evaluation`. Lê um run de Entity Resolution pela API pública dela: a linhagem vem do manifest, a geometria das entidades resolvidas é conferida com a persistida e as afirmações upstream são vinculadas às entidades resolvidas pelas observações espaciais da evidência delas. Só o mapeamento entidade → identidade da avaliação continua sendo uma entrada, à espera da avaliação de identidade de Entity Resolution. Toda a verificação usa fixtures sintéticos, sem run de relações sobre dados reais nem reference set real de relações.
