...
- Schema do `PerceptionRunArtifact` 0.5.0 → 0.6.0: todo run grava `outputs/region-discovery-audit.jsonl`, um registro por frame com os passes de Region Discovery e os diagnostics do backend em cada um, cada candidato rejeitado com o motivo, cada decisão de merge e o digest da configuração de normalização (#611). As rejeições e os merges deixam de se perder ao fim do run. A mesma versão, ainda não lançada, traz mais duas mudanças (#619): cada registro de `metrics/stage-timings.jsonl` ganha `error_type` e `error_traceback`, sempre presentes e `null` fora de `FAILED`, para que um estágio que falhou guarde o tipo da exceção e o traceback truncado; e `raw_response_reference` passa a nomear o stream contratual que guarda a resposta bruta (`outputs/semantic-interpretations.jsonl` para uma execução, suas claims e seu contexto de cena, `outputs/semantic-interpretation-failures.jsonl` para uma resposta rejeitada), materializado pelo `PerceptionRunWriter` na persistência, em vez da cópia em `debug/`, que só existia em debug `full`. O `PerceptionRunReader` continua abrindo runs 0.5.0: informa que a auditoria deles não foi registrada, em vez de devolvê-la vazia, e lê a referência à resposta bruta deles como foi gravada.
...
- Feature Extraction: `ClipConfig` e `AlphaClipConfig` ganham `max_batch_size` (default 32), e os runtimes codificam as regiões de uma cena em lotes desse tamanho, na ordem das regiões, em vez de um único lote do tamanho da cena. O parâmetro entra no fingerprint da configuração, então o fingerprint de toda configuração CLIP e AlphaCLIP muda (#617).
- Region Discovery: `model_version` passa a ser obrigatório em `Sam2Config`, `Sam3Config` e `Florence2Config`; o default `"unknown"` ia para a provenance e o digest no lugar da versão real, e uma configuração sem o parâmetro agora é recusada na composição. Antes da primeira inferência, os runtimes oficiais conferem o device e o dtype do modelo carregado contra a configuração: o `from_model` do SAM2 e o runtime do Florence-2, que não aplicam autocast, exigem pesos em `precision`; o runtime do SAM3 aceita pesos `float32` ou no dtype do autocast configurado (`float16`/`bfloat16`) e só `float32` quando `precision="float32"`. O Florence-2 passa a converter os inputs de ponto flutuante para o dtype verificado (#617).
