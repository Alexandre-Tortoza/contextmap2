...
from contextmap.ingestion.adapters import _ros_common, ros2_bag
...
def test_unsupported_point_field_datatype_becomes_a_readable_warning(tmp_path: Path) -> None:
    path = tmp_path / "with_unsupported_point_field_bag"
    _build_bag(path, include_unsupported_point_field=True)
    config = SourceAdapterConfig(
        source_type="ros2_bag", path=str(path), topics=SourceTopicMapping(lidar="/ouster/points")
    )
    adapter = Ros2BagSourceAdapter(config)
    ...
    assert warning.topic == "/ouster/points"
...
def test_structural_decode_error_propagates_instead_of_becoming_a_warning(
    ...
    config = SourceAdapterConfig(
        source_type="ros2_bag",
        path=str(bag_path),
        topics=SourceTopicMapping(lidar="/ouster/points"),
    )
    adapter = Ros2BagSourceAdapter(config)
...
def test_a_windowed_read_opens_the_bag_only_twice(
    ...
    monkeypatch.setattr(ros2_bag, "Reader", CountingReader)
...
def test_repeated_identical_camera_info_is_kept_once_during_the_scan(
    ...
    adapter = Ros2BagSourceAdapter(
        SourceAdapterConfig(source_type="ros2_bag", path=str(path), topics=_TOPICS)
    )
...
def test_an_unrecognized_distortion_model_is_reported_as_a_warning(tmp_path: Path) -> None:
    ...
    path = tmp_path / "unknown_model_bag"
    ...
    assert warning.topic == "/camera/color/camera_info"
