"""Real context_map assembly, regenerated a fourth time (run-0004) after the ContextMapExecutor
configuration_fingerprint fix (PR #438 review, third round).

Reuses run-0003's still-valid state_estimation..spatial_relations chain unchanged by reference
(only context_map's own assembly logic changed); no need to redo the rest of the chain. Also
exercises the #438 review's fix #3 (ContextMapExecutor now opens and verifies the ``sequence``
input against the geometry it was built over) on real data.
"""
...
OUTPUT_DIR = WORKSPACE / "e2e-real/run-0004/context_map"
...
def main() -> None:
    request = StageRequest(
        ...
        components={},
        config_digest="e2e-real-canonical-run-0004",
        output_dir=OUTPUT_DIR,
        workspace=WORKSPACE,
    )

    executor = ContextMapExecutor(
        up_axis=AxisDirection.POSITIVE_Z, code_version="e2e-real-canonical-run-20260926"
    )
    ...
    print("configuration_fingerprint:", manifest.configuration_fingerprint)
    assert manifest.configuration_fingerprint is not None, (
        "runtime.provenance_identity requires a real configuration digest, not None"
    )
