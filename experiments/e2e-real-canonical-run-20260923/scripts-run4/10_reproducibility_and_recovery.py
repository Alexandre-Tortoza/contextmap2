"""Real, journaled run_plan()/resume_plan() execution, regenerated under the fixed #555 lineage
contract (PR #438 review rerun of issue #182's evidence).

Identical to the original script except for one addition: after the resumed run completes, it
reopens the fresh state_estimation manifest and asserts explicitly that
auxiliary_sequence_artifact_id names the real corridor-02-gt.txt pose artifact -- the same
objective check added to 01_state_estimation.py, so this run's own lineage evidence is not just
implied by "the run finished", here too.
"""
...
from contextmap.ingestion import FrameId, SequenceArtifactId, SequenceArtifactReader
...
def _executors() -> dict[str, Any]:
    ...
    return {
        ...
        "semantic_mapping": SemanticMappingExecutor(
            ...
            code_version="e2e-repro-check-20260926",
        ),
        ...
        "context_map": ContextMapExecutor(up_axis=UP_AXIS, code_version="e2e-repro-check-20260926"),
    }
...
def _scope(hashes: dict[str, str]) -> tuple[Any, Any]:
    ...
    path = Path("/tmp/repro-check-config-run4.json")
...
def main() -> None:
    ...
    store = FileArtifactStore(
        WORKSPACE / "e2e-real/repro-check-index-run4",
        verify=lambda ref: ref.location is not None and (WORKSPACE / ref.location).is_dir(),
    )
    policy = ReusePolicy(store=store, code_identity="corridor-02-repro-check-4")

    failing = _executors()
    failing["semantic_fusion"] = _Failing(failing["semantic_fusion"], ["injected interruption (issue #182, third rerun, after the ContextMapExecutor configuration_fingerprint fix)"])
    ...
    # Objective #555 lineage evidence for this journaled run too, not just implied by success.
    state_estimation_dir = WORKSPACE / refs["state_estimation"].location
    manifest = StateEstimationRunReader(state_estimation_dir).manifest
    print("state_estimation schema_version:", manifest.schema_version)
    print("state_estimation auxiliary_sequence_artifact_id:", manifest.auxiliary_sequence_artifact_id)
    assert manifest.auxiliary_sequence_artifact_id == SequenceArtifactId(
        "e2d832c152b1493999082d4f67210b5b"
    ), f"expected the pose artifact, found {manifest.auxiliary_sequence_artifact_id!r}"
    print("ASSERTION PASSED: journaled run's trajectory also names the real pose artifact")
