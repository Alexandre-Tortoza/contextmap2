"""Assemble the regenerated final AcceptanceReport for Solution 1 / corridor-02 (PR #438 review).

Supersedes the original acceptance-report.json/md: those were built over a StateEstimationRun
Artifact produced under schema_version "0.1.0", whose TrajectoryProvenance never recorded which
artifact its merged poses actually came from. This report is built entirely over run-0002 (the
regenerated state_estimation..context_map chain) and the run-0003/run-0004 journaled rerun, both
under schema_version "0.2.0", where every gate that depends on state_estimation's lineage is
re-earned with fresh evidence rather than carried over.
"""
...
RESULTS = [
    GateResult.passed(
        ...
        detail="Real bag SequenceArtifact 720a486d... (unchanged, reused): verify_integrity()==[], "
               "RGB+LiDAR+IMU under the declared corridor-02-header clock, MeiCameraModel "
               "calibration identity, selection_identity matches the frozen scenario 1.0.4.",
    ),
    GateResult.passed(
        "state_estimation.trajectory_coverage", evidence_class=REAL,
        evidence_refs=("outputs/e2e-real/run-0002/state_estimation", "poses=5522", "gaps=0",
                       "schema_version=0.2.0",
                       "auxiliary_sequence_artifact_id=e2d832c152b1493999082d4f67210b5b"),
        detail="Regenerated real StateEstimationRunArtifact (PR #438 review fix #1): every "
               ...
               "graph map->epson. Unlike run-0001 (schema 0.1.0), the manifest now explicitly "
               "names auxiliary_sequence_artifact_id/auxiliary_selection_id as the real "
               "corridor-02-gt.txt pose artifact -- asserted objectively by "
               "scripts-run2/01_state_estimation.py, not just implied by the run finishing.",
    ),
    ...
    GateResult.passed(
        ...
        evidence_refs=("outputs/e2e-real/run-0002/geometric_mapping",
                       "cross_stage.coordinate_consistency: 1160714 assertions, 0 findings"),
        detail="Regenerated real GeometricMapArtifact over the fixed trajectory: "
    ...
    GateResult.passed(
        ...
        detail="Real PerceptionRunArtifact (SAM2.1-hiera-tiny + DINOv2-base + CLIP-vit-large-"
               "patch14 + Qwen3-VL-4B-Instruct nf4), reused unchanged from run-0001: this "
               "artifact's own schema was not touched by the #438 review fixes, and finding #2 "
               "(VisualPerceptionExecutor's semantic-evidence loss in the composed runtime) is "
               "already covered by a dedicated integration test (test_runtime_composition.py), "
               "not by regenerating ~12 minutes of real GPU inference for this rerun. All 20 "
               "selected frames have a materialized PerceptionResult with an explicit status per "
               "stage; the 44/120 semantic-interpretation failures are recorded via StageOutcome, "
               "never dropped.",
    ),
    GateResult.blocked(
    ...
    GateResult.passed(
        ...
        evidence_refs=("outputs/e2e-real/run-0002/sensor_association",
    ...
    GateResult.passed(
        ...
        evidence_refs=("outputs/e2e-real/run-0002/semantic_fusion",
    ...
    GateResult.passed(
        ...
        evidence_refs=("outputs/e2e-real/run-0002/entity_resolution",
        ...
        detail="Regenerated real EntityResolutionRunArtifact: verify_integrity()==[], 170 "
               "entities (identical to run-0001's science); every resolved entity lists its "
               "member entities/fused evidence, and the rerun cross-stage check found zero "
               "lineage-closure findings across every dependency edge, including the auxiliary "
               "pose sequence's own edge -- the exact edge the #438 review found untested before.",
    ),
    GateResult.blocked(
    ...
    GateResult.passed(
        ...
        evidence_refs=("outputs/e2e-real/run-0002/spatial_relations",
                       "cross_stage.evidence_traceability: 58795 assertions, 0 findings"),
    ...
    GateResult.blocked(
        ...
        detail="Needs AnnotationFamily.RELATIONS, which does not exist for corridor-02 yet, so "
               ...
               "substitute for this gate, identical to run-0001's science): of 10852 total "
    ...
    GateResult.passed(
        ...
        evidence_refs=("sha256:34ed5e9143cba26d0be5a44b3c8377f04094108945b4204d870dd4788425ae33",
                       "outputs/e2e-real/run-0002/context_map"),
        detail="Regenerated real ContextMapArtifact 4379a396...: ContextMapArtifactReader.open("
               "verify_hashes=True) succeeds, entity_count=170, relation_count=10852 (identical "
               "science to run-0001), dependency closure names all 6 real upstream artifacts by "
               "content identity; geometry is referenced, never copied. Also exercises the #438 "
               "review's fix #3 (ContextMapExecutor now opens and verifies the sequence input "
               "against the geometry's own lineage) on real data.",
    ),
    GateResult.passed(
        "cross_stage.lineage_closure", evidence_class=REAL,
        evidence_refs=("experiments/e2e-real-canonical-run-20260923/scripts-run2/09_cross_stage.py",),
        detail="32 real assertions over the real chain (ingestion through ContextMapArtifact), "
               ...
               "CrossStageInputs.auxiliary_sequence is now supplied and verified: the real "
               "corridor-02-gt.txt pose SequenceArtifact that state_estimation actually merged "
               "in is confirmed to match what the trajectory's manifest names, closing the exact "
               "gap the #438 review found (the auxiliary source was previously never checked).",
    ),
    GateResult.passed(
    ...
    GateResult.passed(
        ...
        evidence_refs=("outputs/e2e-real/run-0002/state_estimation/manifest.json#estimator",
        ...
        detail="Spot-checked real manifests record exact backend identity and effective-"
               "configuration digest (state_estimation.manifest.estimator={backend_id: "
               "external_pose, backend_version: 1, ...}; the manifest also now records "
               "auxiliary_sequence_artifact_id/auxiliary_selection_id explicitly). visual_"
               "perception's manifest records a real configuration_digest and each backend's "
               "checkpoint/revision. Not exhaustively re-verified for all 8 stages in this pass.",
    ),
    GateResult.passed(
        ...
        detail="Real wall-time/peak-RSS/storage/file-count measured per stage (issue #181), "
               ...
               "lineage-schema defect the #438 review found, so it needed no rerun. REPORT gate, "
               "no threshold: reported as measured, including the concerning finding that "
               "sensor_association peaks at 24.96 GB RSS (~64% of this 39 GB machine), a real "
               "headroom risk flagged as a limitation, not hidden.",
    ),
    GateResult.passed(
        "reproducibility.rerun_equivalence", evidence_class=REAL,
        evidence_refs=("outputs/e2e-real/run-0002/context_map",
                       "outputs/corridor-02-repro-check/run-0004/context_map"),
        detail="Two independent real reruns under the corrected schema_version 0.2.0, both from "
               "the same real upstream inputs (main bag + pose SequenceArtifacts + reused "
               "PerceptionRunArtifact): (1) this report's own hand-chained run-0002 "
               "(state_estimation..context_map called directly per stage); (2) a separate "
               "run_plan()/resume_plan()-orchestrated rerun (run-0003/run-0004, real interruption "
               "injected, see next gate). Both produced entity_count=170/relation_count=10852, "
               "bit-for-bit matching run-0001's original science and each other; content_identity "
               "differs between the two only because run/config identity strings differ, as "
               "expected. Both also now name auxiliary_sequence_artifact_id="
               "e2d832c152b1493999082d4f67210b5b explicitly in their state_estimation manifest, "
               "asserted objectively by the scripts, not just implied. visual_perception (Qwen "
               "generation) was NOT independently re-verified for reproducibility in this pass -- "
               "greedy/temperature=0 decoding is assumed deterministic on the same "
    ...
    GateResult.passed(
        ...
        evidence_refs=("outputs/corridor-02-repro-check/run-0003", "outputs/corridor-02-repro-check/run-0004"),
        detail="Real run_plan()/resume_plan() execution over the real corridor-02 artifacts "
               ...
               "PipelinePlan.scope(provided=...), never re-derived), regenerated under the fixed "
               "code: a real RuntimeError was injected at semantic_fusion after "
               "state_estimation/geometric_mapping/sensor_association genuinely completed. The "
               "interrupted run's status is 'failed', semantic_fusion's directory was never "
               "created, no .tmp-* leftovers anywhere. resume_plan() then reused the three "
               "completed real stages strictly by reference (their ArtifactRef.location still "
               "points at run-0003, not recomputed), recomputed semantic_fusion onward fresh into "
               "run-0004, and the resumed run's status is 'completed'.",
    ),
]
...
def main() -> None:
    ...
    stage_artifacts = {
        ...
        "state_estimation": ArtifactRecord(
            artifact_id="ee9cf3f2702f9ff1b18a74f7dcadc5fb",
            digest="sha256:020bae7451a493573e64a0754fa4a7363041045cb715e18c2a7a5def0773f273",
        ),
        "geometric_mapping": ArtifactRecord(
            artifact_id="66ad39c22da8ee74384220942337e535",
            digest="sha256:6c58cc05298d20a0332d1a5d6480b112049080254e3c74daed9f0956a0c07376",
        ),
        "sensor_association": ArtifactRecord(
            artifact_id="d52df690e80bc18041c07389d2e9037a",
            digest="sha256:1b56864568a8d1207f5277d5bab72f217f12587cdc87627371d1eb97fa8c862a",
        ),
        "semantic_fusion": ArtifactRecord(
            artifact_id="53adefa1fd687fe833af23c6378e70c2",
            digest="sha256:4061149e68183a2e7d10111db893fb7400bddf90921e29260b2fac4df92ccb7b",
        ),
        "semantic_mapping": ArtifactRecord(
            artifact_id="9a21d63dd6d835b64ac923096693c700",
            digest="sha256:14f2719c2a65cb7e33759f147f9360d3c2ee695b8271fde4d705372e6ded9be4",
        ),
        "entity_resolution": ArtifactRecord(
            artifact_id="bcd9cb4c966ae41693efb3cf0d6a8e88",
            digest="sha256:7dffa7b2e7d45ce445acf0cc4753542196568924e4bb1ce2657c6f2184bd03e5",
        ),
        "spatial_relations": ArtifactRecord(
            artifact_id="079c8cbdeb4171ff2841e00c45fea002",
            digest="sha256:11ca0c74b0e99193d6c39d729ab66db247174ac359913ebadff99186b3f54176",
        ),
        "context_map": ArtifactRecord(
            artifact_id="4379a396fa7323dd88360f45bf7dcbf2",
            digest="sha256:34ed5e9143cba26d0be5a44b3c8377f04094108945b4204d870dd4788425ae33",
        ),
    }
    ...
    report = assemble_acceptance_report(
        scenario,
        report_id="acceptance-report-corridor-02-20260924-run0002",
        run_id="e2e-real-run-0002",
        code_version="0.0.1.dev649+g7915dea8d",
        results=RESULTS,
        stage_artifacts=stage_artifacts,
        final_artifact=final_artifact,
        limitations=(
            "outputs/e2e-real/run-0001/{state_estimation,geometric_mapping,sensor_association,"
            "semantic_fusion,semantic_mapping,entity_resolution,spatial_relations,context_map} "
            "and outputs/corridor-02-repro-check/{run-0001,run-0002} were produced under "
            "StateEstimationRunArtifact schema_version 0.1.0, before the PR #438 review fix: "
            "their TrajectoryProvenance never recorded which artifact the merged poses actually "
            "came from. They are superseded for acceptance purposes by run-0002 and "
            "corridor-02-repro-check/{run-0003,run-0004} respectively, and must not be cited as "
            "evidence for lineage/reproducibility/recovery gates going forward. run-0001's "
            "PerceptionRunArtifact (visual_perception) remains valid evidence: that schema was "
            "not touched by the fix, and it continues to be reused by reference.",
            ...
            "few MB output -- a real algorithmic working-set cost, a real OOM risk on smaller "
            ...
            "corridor-02's real physical scale; the resulting relation counts are unremarkable, "
            "but this remains an un-tuned default, not a deliberately chosen one.",
    ...
    path = Path("/home/alexmrtr/Projects/contextmap2/outputs/e2e-real/acceptance-report-run0002.json")
