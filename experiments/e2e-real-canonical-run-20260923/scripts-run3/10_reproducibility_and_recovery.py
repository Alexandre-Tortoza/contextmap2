...
def _executors() -> dict[str, Any]:
    ...
    return {
        ...
        "semantic_mapping": SemanticMappingExecutor(
            ...
            code_version="e2e-repro-check-20260925",
        ),
        ...
        "context_map": ContextMapExecutor(up_axis=UP_AXIS, code_version="e2e-repro-check-20260925"),
    }
...
def _scope(hashes: dict[str, str]) -> tuple[Any, Any]:
    ...
    path = Path("/tmp/repro-check-config-run3.json")
...
def main() -> None:
    ...
    store = FileArtifactStore(
        WORKSPACE / "e2e-real/repro-check-index-run3",
        verify=lambda ref: ref.location is not None and (WORKSPACE / ref.location).is_dir(),
    )
    policy = ReusePolicy(store=store, code_identity="corridor-02-repro-check-3")

    failing = _executors()
    failing["semantic_fusion"] = _Failing(failing["semantic_fusion"], ["injected interruption (issue #182, second rerun after the code_version fix)"])
