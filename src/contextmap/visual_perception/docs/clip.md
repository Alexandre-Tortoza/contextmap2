...
`REGION` com zero regiões aceitas é um resultado legítimo de Region Discovery (um frame sem nada saliente), não um erro (#380): `extract()`/`extract_visual()` retornam zero features sem invocar o runtime, `ClipExtraction.embedding_space` fica `None` (nenhuma view foi codificada, logo não há identidade de espaço a relatar) e `array` fica com shape `(0, 0)`. Regiões não aceitas continuam rejeitadas explicitamente.
...
As views de uma chamada são codificadas em lotes consecutivos de no máximo `max_batch_size` (default 32), para que o número de regiões de uma cena densa não defina sozinho a memória de uma inferência. As linhas do array de saída seguem a ordem das views, e o resultado dos lotes é o mesmo do lote único. `max_batch_size` entra no fingerprint da configuração, porque em GPU o tamanho do lote pode mudar o kernel escolhido e o arredondamento em ponto flutuante (#617).
