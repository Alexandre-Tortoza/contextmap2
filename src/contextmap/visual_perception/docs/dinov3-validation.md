...
| Item | Valor |
| --- | --- |
| Código | `dev` @ `ac1eb59` (adapter DINOv3 inalterado; as correções descritas abaixo estão em `feature_diagnostics.py` e `run_artifact.py`; a reverificação do vínculo diagnostic↔feature usou o código corrigido) |
...
## Vínculo da geometria com o payload (revisão da milestone)

A revisão da milestone (#423) apontou que promover `dense` a metadata contratual não bastava: nada garantia que o registro descrevesse a feature persistida. Um diagnostic com grade 4×4 e shape `(4, 4, 4)` era aceito para uma feature persistida com shape `(2, 2)`, e `verify_integrity()` passava, porque só confere presença, tamanho e hash do inventário. O run era íntegro por hash e semanticamente inconsistente.

Correção: `PerceptionRunWriter.finalize()` confere cada diagnostic `SUCCEEDED`/`WARNING` contra exatamente uma `VisualFeature` do run (mesma observação e `feature_id`; scope, shape, dtype, normalização, referência do payload, proveniência do backend, fingerprint do `EmbeddingSpace` e, em região, `region_id`), exige no escopo denso shape `(altura, largura, canais)` com `(grid_height, grid_width) == shape[:2]` e `source_artifact_id` igual ao `run_id` que possui a feature, e rejeita dois diagnostics para a mesma feature. `DenseFeatureDiagnostic` passou a rejeitar origem, stride e suporte não finitos (NaN/Inf), como `DenseFeatureSampling`. Detalhes em [`feature_diagnostics.md`](feature_diagnostics.md). Testes de regressão (falhavam antes; o caso do shape `(2, 2)` era aceito pelo writer): `test_finalize_rejects_a_dense_diagnostic_declared_for_another_persisted_shape`, `test_finalize_rejects_a_dense_grid_that_disagrees_with_the_feature_shape`, `test_finalize_rejects_a_dense_grid_with_swapped_axes`, `test_finalize_rejects_a_dense_feature_that_is_not_height_width_channels`, `test_finalize_rejects_dense_geometry_owned_by_another_run`, `test_finalize_rejects_a_diagnostic_whose_metadata_differs_from_its_feature`, `test_finalize_rejects_a_region_diagnostic_bound_to_another_region`, `test_finalize_rejects_a_produced_diagnostic_without_its_feature`, `test_finalize_rejects_two_diagnostics_for_the_same_produced_feature`, `test_dense_diagnostic_rejects_non_finite_stride_and_support` e `test_dense_diagnostic_and_dense_sampling_accept_the_same_geometry`.

**Reverificação com pesos reais.** A nova validação não pode rejeitar a saída real que esta milestone validou. Com o código corrigido, `facebook/dinov3-vits16-pretrain-lvd1689m` @ `114c1379…`, 3 frames reais de `corridor-02` (os três primeiros de `frames.json`), `float32`, `cuda`, 448×336 e `FeatureDebugLevel.NONE`, o mesmo padrão do driver `30_run_artifact` gravou um run em um workspace descartável: `finalize()` aceitou os três diagnostics (feature `(21, 28, 384)`, `source_artifact_id` igual ao `run_id`), `verify_integrity()` retornou sem problemas, o payload reaberto é idêntico ao da memória em 3/3 e o `DenseFeatureSampling` reconstruído de `metrics/feature-extraction.jsonl` (mais o `DenseFeatureMap` com a feature reaberta) coincide com o da memória em 3/3. Controles negativos com os mesmos objetos reais: uma coluna a mais na grade (`(21, 28)` contra `(21, 29)`), outro run como dono e o shape `(2, 2)` declarado são rejeitados pelo `finalize()`. É uma verificação de contrato com dados reais em amostra pequena, não uma nova avaliação de qualidade.
