...
Contrato único de geometria 2D consumido por todo Visual Perception. `region_id` é local ao `PerceptionResult`; o próprio resultado fornece os escopos de run e observação. `bounding_box`, máscara inline opcional ou `mask_reference`, dimensões, área, proposal contributors e `discovery_provenance` preservam a geometry freeze e a auditoria sem criar outra classe `Region2D`. O `BackendProvenance` é preservado diretamente do adapter durante a normalização, sem inferir provider a partir do `backend_id`. `mask` continua disponível em memória enquanto a região trafega dentro de uma execução (normalização, Sensor Association no mesmo processo). Um `Region2D` rejeitado permanece auditável via `rejection_reason`.
