...
- `SemanticViewPolicy`, `SemanticContextBoundary`, `SemanticViewConstruction`, `MaterializedSemanticView`, `materialize_region_views()`/`materialize_scene_view()` e `check_view_policy_supported()` — política versionada de views de evidência, a materialização determinística e content-addressed das views e o preflight contra as capacidades do intérprete ([Política de views de evidência](semantic-interpretation.md#política-de-views-de-evidência-524)).
- `SemanticRequestPolicy`, `SemanticModePrompt`, `SEMANTIC_REQUEST_POLICY_VERSION` e `check_request_policy_supported()` — a política de request resolvida de um run (prompt por modo, views ordenadas, contexto de cena), com uma identidade ([Política de request resolvida](semantic-interpretation.md#política-de-request-resolvida-544)).
- `SCENE_CONTEXT_RENDERING` — forma versionada em que um template de região renderiza o contexto de cena que condiciona o request ([Condicionamento por contexto de cena](semantic-interpretation.md#condicionamento-por-contexto-de-cena-529)).
