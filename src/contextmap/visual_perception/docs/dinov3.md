...
- **persistência:** um run imutável reabre com integridade íntegra, payload idêntico e, com `FeatureDebugLevel.NONE`, o `DenseFeatureSampling` é reconstruído exatamente de `metrics/feature-extraction.jsonl`. O `finalize()` só publica o run se cada diagnostic descrever a feature persistida (grade igual a `shape[:2]`, `source_artifact_id` igual ao `run_id`, mesmo `EmbeddingSpace`, dtype, normalização, payload e proveniência); ver [`feature_diagnostics.md`](feature_diagnostics.md).
