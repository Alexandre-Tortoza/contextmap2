...
- `generate_relation_candidates()`, `CandidatePolicy`, `RelationCandidate`, `RelationCandidateSet`, `CandidateExclusion`, `CandidateExclusionSummary`, `CandidateReason`, `CandidateExclusionReason`, `SkippedPredicate`, `CandidateProvenance`, `CANDIDATE_POLICY_ID` — a redução determinística dos pares a avaliar, com as razões de exclusão inspecionáveis e, acima do teto por grupo `(predicado, razão)`, o resumo auditável das exclusões não listadas.
...
- `link_statements()`, `UpstreamRelationStatement`, `LinkedStatements`, `UnlinkedStatement`, `LinkFailure` — o vínculo de afirmações upstream às entidades resolvidas, pela resposta do leitor do run de Entity Resolution (`resolved_of_spatial_observation`) sobre as observações espaciais, sem palpite.
...
- `lineage_from_resolution_manifest()`, `resolved_entity_geometries()` — a leitura de um run de Entity Resolution pela API pública dela: a linhagem derivada do manifest (com o digest calculado por ela) e a geometria das entidades resolvidas conferida com a persistida.
