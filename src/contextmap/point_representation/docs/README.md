...
Existem os **contratos** (`PointRepresentation`, `RepresentationSpace`, política e suporte espacial, coordenadas preparadas), sua serialização, a **extração de suporte** local sobre `GeometrySource` (`SupportExtractor`) e a porta `PointEncoder` com o serviço de execução independente de backend; o **descritor geométrico determinístico** (`backends/geometric_descriptor.py`, o encoder de controle) implementa a porta sem nenhuma dependência de modelo. A **fronteira do backend opcional PTv3** (`backends/ptv3.py`) existe e é verificada com um runtime falso; o **runtime real sobre Pointcept** (`backends/ptv3_pointcept.py`) está implementado, coberto por testes determinísticos e por uma suíte opt-in em GPU, e teve tempo (30 ms por suporte) e memória (pico de 193 MiB de tensores) **medidos em geometria real** (ver [`ptv3.md`](ptv3.md)). O **`PointRepresentationRunArtifact`** persiste os runs de forma imutável, com payload lazy e integridade (ver [`artifact.md`](artifact.md)). O **harness de avaliação** vive em `contextmap.evaluation` (ver [`point_representation.md`](../../evaluation/docs/point_representation.md) na documentação de `evaluation`); a ablação real contra `off` e o descritor está registrada, e o efeito downstream continua pendente.
...
- [`artifact.md`](artifact.md) — o run artifact: layout, payload lazy, linhagem, integridade, níveis de debug, decisões e medições.
