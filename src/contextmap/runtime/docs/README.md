...
Existe ainda a **CLI** (issue #166), uma camada fina que traduz flags em overrides e chama esses serviços: `run`, `stage`, `inspect` e `validate`, com dry-run que mostra o plano resolvido sem carregar modelo, saída `--json`, erros acionáveis e verificação de integridade só com a biblioteca padrão, exceto um `ContextMapArtifact`, que `validate` e `inspect artifact` entregam ao validador da capability `artifact` (issue #603). Detalhes em [`cli.md`](cli.md).
