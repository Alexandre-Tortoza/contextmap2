...
- **`semantic_mapping` sem componente de catálogo e sem executor.** É um estágio real de `canonical/1`, mas ainda roda inteiramente fora da composição automática; sua entrada em `entity_resolution` só existe hoje via artifact suprido.
- **A montagem do `ContextMapArtifact` (`context_map`).** Ainda não é um estágio de `canonical/1`; é tratada por outro milestone (End-to-End Validation), que também reconcilia `catalog.py`/`composition.py`/`executors.py` com o que este documento descreve.
