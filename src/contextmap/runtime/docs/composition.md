...
Isto é uma bridge incremental para a #177, não a arquitetura-alvo de "uma aquisição física = um `SequenceArtifact`": carregar só o artifact principal continua sem revelar a trajetória gravada. `StateEstimationRequest`/`TrajectoryProvenance` não ganham nenhum campo novo — a fusão dos dois artifacts é decisão só do executor:

- `StateEstimationExecutor.execute()` lê `pose_sequence` só quando presente (`_optional_one`, o par opcional de `_one`) e, antes de combinar qualquer observação, chama `contextmap.ingestion.validate_cross_source_clock_compatibility()` sobre as duas sequências — nunca confia que os dois `timestamp_clock_id` batem só porque a string é igual; um intervalo de tempo disjuntos (o mesmo padrão de defeito que a #554 corrigiu) recusa o merge com um erro explícito.
- o `pose_role` declarado pela sequência auxiliar (`ground_truth`/`odometry`/`external_localization`, ver `adapters.md`) precisa ser único e consistente entre todas as suas observações; ausente ou divergente também recusa o merge.
- `pose_role="ground_truth"` nunca vira trajetória operacional por padrão: suas observações são descartadas do merge (o run se comporta exatamente como se a sequência auxiliar não existisse) a menos que `policies.trajectory_mode = "allow_ground_truth"` seja escolhido explicitamente na configuração — `_compose_state_estimation` repassa essa política como `StateEstimationExecutor(..., allow_ground_truth_trajectory=...)`, nunca inferida da simples presença do artifact ou da escolha do backend `external_pose`.
- a linhagem de que dois artifacts contribuíram já é automática: `pose_sequence` é um input nomeado do estágio, então o mecanismo de referência por estágio que a runtime já tem (`ArtifactRef`/`StageRequest.inputs`) grava os dois, sem precisar de nenhum id novo no domínio de `state_estimation`.
