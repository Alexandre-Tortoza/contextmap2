...
- **`ingestion`, `visual_perception` e `point_representation` sem composição automática.** `compose_executors` monta `state_estimation`, `geometric_mapping`, `sensor_association` e `semantic_fusion` da configuração; `ingestion` precisa de um `IngestionRequest` que não é parte de nenhuma configuração (injete um `IngestionStageExecutor`, ou rode `contextmap ingest` e forneça/selecione o artifact publicado), e `visual_perception`/`point_representation` ainda não têm executor real (dependem de modelo/GPU) — os testes desta fachada usam estágios falsos para essas três.
