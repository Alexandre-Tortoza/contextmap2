...
- **`point_representation` sem executor real.** `contextmap.runtime.executors` tem executores reais para `state_estimation`, `geometric_mapping`, `sensor_association`, `semantic_fusion` e, desde #507, `visual_perception` (ver [`executors.md`](executors.md)), e a [composition root](composition.md) os monta automaticamente da configuração (`compose_executors`). `point_representation` depende de backend com modelo/GPU e ainda não tem um; um `targets=[...]` que o inclua precisa de um executor injetado ou fica bloqueado no preflight, explicitamente — nunca simulado.
