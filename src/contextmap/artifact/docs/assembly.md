...
A configuração efetiva não é descartada — ela pertence à linhagem do run, não ao `PolicyRef` de cada entidade/relação: `configuration_fingerprint` de `UpstreamArtifact` documenta exatamente "hash da configuração efetiva que o produziu". `_resolution_upstream_artifact` cita `materialization().policy.configuration_fingerprint`, o único papel (role) desse run que a montagem de fato lê; `_relations_upstream_artifact` cita a `configuration_fingerprint` da primeira relação do run (`decide_relations` é uma chamada determinística sobre todo o conjunto de candidatos, então toda relação de um run genuinamente compartilha a mesma configuração efetiva — um run sem relações não tem o que citar, e fica `None`). `taxonomy_version` não tem um campo correspondente em `contextmap.artifact.PolicyRef`: como os demais campos que `docs/assembly.md` já documenta como não copiados (`attributes`, `uncertainty` detalhado), continua alcançável, sem alteração, no próprio registro que `ContextRelation.source_run_id`/`source_relation_id` cita.
...
`AssemblyMetrics` separa `translation_duration_seconds` (abrir os runs a montante e traduzir seus registros, até — sem incluir — construir o `ContextMap`) de `validation_duration_seconds` (a própria construção do `ContextMap`, que é exatamente quando `composition.py` e `_lineage_checks.py` rodam, dentro de `__post_init__`): essa é a "validação" real da issue #537, não um passo extra inventado. Esta função nunca escreve em disco, então **não há uma duração de escrita separada para relatar aqui** — persistir o mapa continua sendo `ContextMapArtifactWriter`, que roda depois e mediria sua própria duração se e quando a issue #537 exigir isso do lado da escrita; esta é uma lacuna conhecida, registrada aqui em vez de silenciada. `peak_memory_bytes` usa `tracemalloc` (biblioteca padrão), a única medição de pico de memória de CPU comparável já usada no repositório sendo específica de backends de modelo (`torch`/GPU), não aplicável a uma composição determinística como esta. `entity_count`, `relation_count` e `lineage_count` são lidos do próprio `ContextMap` já validado.
...
Também fora de escopo, registrado explicitamente em vez de silenciado (issue #541, achado 6): uma duração de escrita separada, já que `assemble_context_map`/`assemble_context_map_with_metrics` nunca escrevem em disco — só `ContextMapArtifactWriter` faz isso, e não tem hoje sua própria instrumentação de tempo/memória. Medir a escrita fica para quando um estágio de runtime `context_map` (o item de escopo futuro acima) de fato encadear montagem e escrita em uma única execução observável.
