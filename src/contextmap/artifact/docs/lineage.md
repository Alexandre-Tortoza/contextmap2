...
- `ENTITY_RESOLUTION_RUN` foi acrescentado a `MULTIVIEW_FUSED` para a montagem real do `ContextMap` (`serialization/assembly.py`, [#537](https://github.com/Alexandre-Tortoza/contextmap2/issues/537)): a materialização de Entity Resolution também é uma regra versionada que acumula várias contribuições de evidência — a de cada membro fundido — e é o único artifact que a montagem (que nunca abre um run de Semantic Fusion) tem de fato aberto para citar essa derivação com honestidade. `SEMANTIC_FUSION_RUN` continua a citação correta para um resultado fundido a partir de uma leitura real desse run (a fixture representativa em `validation.md` continua usando-o), e é exatamente o que a montagem real cita a partir da issue [#541](https://github.com/Alexandre-Tortoza/contextmap2/issues/541): `ContextEntity.origin` cita `ENTITY_RESOLUTION_RUN` (a materialização identificou este `ResolvedEntity`), mas cada `LabelHypothesis.origin` cita `SEMANTIC_FUSION_RUN` (o rótulo em si veio de uma evidência fundida, não da resolução de identidade) — dois registros diferentes, nunca o mesmo, mesmo quando a montagem nunca abre o run de Semantic Fusion diretamente (o `SEMANTIC_FUSION_RUN` da linhagem é derivado de graça do `FusedEvidenceRef` que o `ResolvedEntity` já carrega; ver `docs/assembly.md`).
...
- **Relação**: `origin.derived_from` cita a evidência da relação (os `RelationEvidenceId` de Spatial Relations e a geometria de suporte, como `UpstreamRecordRef`) — nunca só o próprio registro da relação, que `source_run_id`/`source_relation_id` já nomeiam estruturalmente — e, quando a relação é derivada (inversa ou simétrica), a relação de origem também entra em `derived_from` (issue [#541](https://github.com/Alexandre-Tortoza/contextmap2/issues/541)); `origin.policy` registra a política de decisão (não a versão da taxonomia, que é um vocabulário separado — ver `docs/assembly.md`).
