...
A passada de hash (`file_hashes`) lê do disco, em blocos, cada arquivo inventariado. Fora ela, cada arquivo das tabelas (`entities.jsonl`, `relations.jsonl` e os três índices) é lido **no máximo uma vez** por validação (#600): como o artifact só é lido, os bytes lidos são compartilhados por `index_structure`, `reference_integrity` e `index_rebuild`, e cada tabela é aberta uma vez só, sobre o índice já lido. Antes eram três leituras de cada payload e quatro de cada índice de deslocamento. `index_rebuild` é a última verificação que lê as tabelas, então os bytes delas são soltos antes de `schema_invariants` montar o mapa decodificado.

O preço é segurar os bytes das tabelas de `reference_integrity` até `index_rebuild`, junto dos registros crus, que continuam residentes até `schema_invariants`. Medido numa máquina de desenvolvimento com um mapa sintético de 8 000 entidades e 16 000 relações (25 MiB de tabelas), o pico do `tracemalloc` de uma validação `FULL` passou de 148,2 MiB para 161,4 MiB; o tempo não mudou, porque é dominado pela decodificação dos registros. `test_full_validation_reads_each_table_once_beyond_the_hash_pass` conta as aberturas de cada arquivo das tabelas, e relatórios gravados antes da mudança (`test_the_reports_match_the_recorded_ones`) provam que a ordem, os resultados e os achados das verificações são os mesmos.
