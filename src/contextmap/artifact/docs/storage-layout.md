...
- `content_identity` de um artifact a montante é o **digest de artifact** que os irmãos já usam para se fixar uns aos outros (Semantic Fusion, Semantic Mapping, Entity Resolution, Spatial Relations): o SHA-256 de `{run_id, schema_version, arquivos: [[caminho, hash]…]}` ordenado, calculado do `manifest.json` do próprio artifact por `artifact_digest(diretório)`. É o mesmo valor que Spatial Relations grava como `entity_resolution_artifact_digest` do run de Entity Resolution, então a linhagem do mapa e a do run a montante concordam sobre a identidade do mesmo artifact. Ele ignora tamanhos, horário de escrita e tudo que não é contratual. Torna a referência **exata**: um artifact achado por qualquer caminho só é a dependência se o digest bater. O writer confere essa igualdade para todo artifact localizado, e nenhum caminho absoluto é gravado.
...
Para cada versão prometida como legível pela v0.1.0 há uma **fixture** versionada em `tests/fixtures/context_map_artifact/<versão>/`, gravada pelo writer e **congelada a partir da release `v0.1.0`** (enquanto a release não sai, o schema ainda muda e a fixture é regenerada no mesmo PR que o muda, como fez a ressincronização com o schema de Entity Resolution e Spatial Relations; sem camada de compatibilidade em v0.x): `v0.1.0` é um artifact completo (formato `0.1.0`, schema `0.1.0`), montado dos runs **reais** de Entity Resolution e Spatial Relations da cena de teste: três entidades resolvidas (uma fundida de dois membros, com decisão e vizinho não resolvido), dez relações (quatro `SUPPORTED`, seis `REJECTED`) e estados semânticos ambíguo e sem evidência. Um teste fixa a `content_identity` da fixture, então regenerá-la por engano falha (quem a regenera troca a constante de propósito), e outros testes garantem que ela continua abrindo com hash verificado, que os registros são iguais aos do schema e que o validador completo só aponta o que a fixture não carrega (as dependências a montante, que são grandes e não versionadas). Quando uma versão de formato ou de schema nova for prometida como legível, acrescenta-se a sua fixture ao lado, sem tirar a anterior; quando uma deixar de ser legível, a mensagem de rejeição é a que a fixture antiga passa a exercitar.
