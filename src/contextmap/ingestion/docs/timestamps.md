...
`encode_timestamp_policy()`/`decode_timestamp_policy()` serializam a política inteira (clocks nomeados + tipo/parâmetros da correção + âncoras, quando houver) dentro de `SequenceProvenance.ingestion_config["timestamp_policy"]`. O offset é gravado como `offset_nanoseconds` inteiro, nunca como segundos em ponto flutuante: um offset de anos em `float64` perde nanossegundos no round-trip, e o decode reconstrói a correção exata a partir desse inteiro (um valor não inteiro é recusado com `ValueError`). Não há leitura da forma antiga `offset_seconds` (v0.x, sem camada de compatibilidade). Tudo isso vive no mesmo mecanismo genérico já usado para tópicos, janela e sincronização, e não num campo novo do schema do manifest. Como `ingestion_config` já alimenta `configuration_hash`/`compute_content_identity()`, uma mudança na política muda a identidade de conteúdo da sequência, exatamente como uma mudança de calibração ou sincronização já mudava antes desta issue: mesma fonte + mesma configuração produz sempre o mesmo tempo normalizado e a mesma identidade de seleção.
