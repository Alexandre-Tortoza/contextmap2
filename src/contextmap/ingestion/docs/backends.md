...
- **Calibração**: todas as mensagens de `camera_info` devem ser equivalentes durante a sequência; mudança de intrínsecos falha explicitamente. `distortion_model == "equidistant"` vira `FisheyeCameraModel`; `plumb_bob` e `rational_polynomial` viram `PinholeCameraModel`. Um `D` equidistante com mais de 4 coeficientes é truncado para `(k1, k2, k3, k4)`, e um com menos é completado com `0.0`; nos dois casos a normalização fica registrada em `CalibrationProvenance.conversions_applied` (quantos coeficientes vieram e o que foi descartado ou completado), além do `D` bruto em `original_values`. Um `distortion_model` não reconhecido vira `PinholeCameraModel` com `DistortionModel.NONE` e o `D` como veio (a validação de calibração acusa uma contagem de coeficientes incompatível); a nota nomeia o modelo, já que com `D=[]` o resultado seria indistinguível de uma câmera sem distorção. Toda nota de conversão de uma calibração descoberta também vira um `SourceAdapterWarning` no tópico `camera_info`, com o índice da mensagem de origem, no início de `warnings()` da leitura que usou essa calibração — e, pelo runtime, nos warnings da proveniência da sequência. A calibração descoberta é mesclada com a fornecida externamente, conflitos por sensor são rejeitados e observações recebem o `calibration_id` aplicável.
