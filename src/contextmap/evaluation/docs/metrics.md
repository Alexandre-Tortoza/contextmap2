...
- `region_discovery_evaluation_report()` — `region.iou.mean`, `region.recall.mean`, `region.duplicate_rate.mean` sobre os frames anotados (frames sem anotação saem da população, não valem zero; em `region.duplicate_rate.mean` também saem os frames anotados sem nenhuma região descoberta, cuja taxa é `null`) e `runtime.wall_time`/`runtime.peak_memory`;
